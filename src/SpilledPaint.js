...
var gooSlider = function (parent, id) {
  ...
  this.wave = function (settings, call) {
    // this creates the wave object. You pass through the color of the "wave" and the callback for when the wave covers the screen.
  ...
  function loop() {
    ...
    for (var i = 0; i <= waves.length; i++) {
      ...
      if (waves[i] != undefined) {
        // make sure it's still getting animated
        ...
        for (var c = 0; c < waves[i].drips.length; c++) {
          // for each drip in the wave
          waves[i].drips[c] +=
            waves[i].settings.minSpeed +
            (noise.simplex2(i * dripsAmount + c, currentTime) + 1) *
          ...
          if (waves[i].drips[c] < size.y) {
            // if this point is not to the bottom of the screen yet, set done to false.  Once all points are below the screen, done will be set to true and the animation for this wave will end and the callback will be... called.
            done = false; // ^^
          }
        }
        ...
        if (done == true) {
          ...
          for (p = 1; p < points.length - 2; p++) {
            // loop through almost all of the points, but leave the last 2 alone and we'll code that in by hand
  ...
  this.clear = function () {
    // this will come in handy for practical uses
    waves = [];
    context.clearRect(0, 0, size.x, size.y);
  };
